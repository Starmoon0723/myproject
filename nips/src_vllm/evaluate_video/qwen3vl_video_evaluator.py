...
class Evaluator:
    def __init__(
        ...
        post_process=False,
        resume=False,
        eval_only=False,  # 只用于评估，不初始化模型和数据
        ...
        video_divprune_exact_threshold=4096,
        fps=2,
        nframe=-1,
    ):
        ...
        self.video_pruning_method = str(video_pruning_method or "evs")
        self.video_divprune_exact_threshold = int(video_divprune_exact_threshold)
        self.fps, self.nframe = self._resolve_video_sampling(fps=fps, nframe=nframe)
        ...
        dataset = self.VIDEO_DATASET_CLS(
            ...
            mm_cache_dir=self.mm_cache_dir,
            use_mm_cache=self.use_mm_cache,
            mm_cache_read_only=self.mm_cache_read_only,
        )
        ...
        if disable_custom_all_reduce:
            logger.info(
                ...
                "(can override via VLLM_DISABLE_CUSTOM_ALL_REDUCE=0)."
            )
        self.llm = LLM(
            ...
            video_divprune_exact_threshold=self.video_divprune_exact_threshold,
        ...
        for char in ["A", "B", "C", "D"]:
            ids = []
            ids.append(tokenizer.encode(char, add_special_tokens=False)[0])  # 尝试编码 "A"
            ids.append(tokenizer.encode(" " + char, add_special_tokens=False)[0])  # 尝试编码 " A"
            self.option_tokens[char] = list(set(ids))  # 去重并保存
    ...
    @staticmethod
    def _resolve_video_sampling(fps, nframe):
        fps = -1 if fps is None else fps
        nframe = -1 if nframe is None else nframe

        if fps == -1:
            if nframe == -1:
                raise ValueError("When fps is -1, nframe must be set to a positive integer.")
            if nframe <= 0:
                raise ValueError("nframe must be > 0 when fps is -1.")
            return None, nframe
        ...
        for idx in tqdm(range(len(self.dataloader)), desc=f"Group {self.group_id} processing {self.dataset_name}"):
            ...
            for o in vllm_outputs:
                ...
                if o.outputs[0].logprobs:
                    ...
                    sample_probs = {}
                    for opt_char, opt_ids in self.option_tokens.items():  # 遍历需要的目标（A, B, C, D）
                        best_logprob = -999.0
                        found = False
                        for tid in opt_ids:  # 在 Top-20 里找这些 ID
                            if tid in top_logprobs:
                                ...
                                if top_logprobs[tid].logprob > best_logprob:
                                    ...
                                    found = True
                        sample_probs[opt_char] = best_logprob if found else None  # Top-20里未命中则置为None
            ...
            for line, ret, logprob_dict, req, model_input in zip(
                batch_line,
                batch_ret,
                batch_logprobs,
                vllm_outputs,
                ...
                if isinstance(video_items, list) and len(video_items) > 0:
                    first_video = video_items[0]
                    if isinstance(first_video, (list, tuple)) and len(first_video) == 2:
                        video_tensor, video_metadata = first_video
                        line["vllm_video_tensor_shape"] = _to_jsonable(
                            getattr(video_tensor, "shape", None)
                        )
                        line["vllm_video_metadata"] = _to_jsonable(video_metadata)
                line["vllm_mm_processor_kwargs"] = _to_jsonable(
                    model_input.get("mm_processor_kwargs", {})
                )
            ...
            logger.info(
                ...
                f"Post {duration_post:.2f}s, Del {duration_del:.2f}s"
            )
    ...
    def merge_results(self):
        ...
        try:
            ...
            temp_dataset = self.VIDEO_DATASET_CLS(
                temp_processor,
                group_id=0,
                num_groups=1,
                use_vllm=True,
