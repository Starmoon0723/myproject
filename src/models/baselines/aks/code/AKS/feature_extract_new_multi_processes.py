...
def parse_arguments():
    ...
    # --- Per-video batching (new, default=1 keeps original per-frame behavior) ---
    parser.add_argument(
        '--frame_batch_size',
        type=int,
        default=1,
        help='batch size for frames within ONE video (effective for CLIP). 1 keeps original per-frame forward.',
    )
...
def _process_one_item(
    *,
    ...
    try:
        ...
        _duration = data.get("duration", None)  # keep original semantics (unused)

        # Keep decode behavior consistent with your current code (CPU decode, 2 threads)
        vr = VideoReader(video, ctx=cpu(0), num_threads=2)
        fps = vr.get_avg_fps()
        step = max(1, int(fps))
        frame_nums = int(len(vr) / step)  # keep original meaning
        ...
        if args.extract_feature_model == "blip":
            ...
            # Keep exactly the same sampling rule as original: indices are j*int(fps)
            frame_indices = [j * step for j in range(frame_nums)]
            frame_num = frame_indices[:]  # keep same output

            bsz = max(1, int(getattr(args, "frame_batch_size", 1)))
            if bsz == 1:
                for j in range(frame_nums):
                    raw_image = np.array(vr[j * step])
                    raw_image = Image.fromarray(raw_image)
                ...
                for start in range(0, len(frame_indices), bsz):
                    batch_idx = frame_indices[start : start + bsz]
                    frames_obj = vr.get_batch(batch_idx)
                    # decord may return NDArray (has asnumpy) OR torch.Tensor (if decord bridge is set to "torch")
                    if hasattr(frames_obj, "asnumpy"):
                        frames = frames_obj.asnumpy()
                    elif isinstance(frames_obj, torch.Tensor):
                        frames = frames_obj.detach().cpu().numpy()
                    else:
                        frames = np.asarray(frames_obj)

                    pil_images = [Image.fromarray(f) for f in frames]
                    inputs_image = processor(images=pil_images, return_tensors="pt", padding=True).to(device)
                    with torch.no_grad():
                        image_features = model.get_image_features(**inputs_image)  # [B, D]

                    sims = F.cosine_similarity(
                        image_features,
                        text_features.expand(image_features.shape[0], -1),
                        dim=-1,
                    )  # [B]
                    score.extend(sims.detach().cpu().tolist())
