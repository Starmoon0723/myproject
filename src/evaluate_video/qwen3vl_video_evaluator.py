...
class Evaluator:
    def __init__(
        ...
        if dataset_name == "Video-MME":
            ...
            raise ValueError("只支持Video-MME、MVBench、MLVU、LVBench、Charades-STA、MMVU、VideoMMMU、FCMBench")
        ...
        if use_vllm:
            ...
            limit_mm_per_prompt = {"image": 64, "video": video_limit}
            self.llm = LLM(
            ...
            if self.use_vllm and self.dataset_name not in ["Charades-STA", "MMVU", "VideoMMMU", "FCMBench"]:
    ...
    def inference(self):
        ...
        for idx in tqdm(range(len(self.dataloader)), desc=f"Group {self.group_id} processing {self.dataset_name}"):
            ...
            if self.use_vllm:
                ...
                if self.dataset_name in ["Charades-STA", "MMVU", "VideoMMMU", "FCMBench"]:
                                ...
                                for tid in opt_ids:     # 在 Top-20 里找这些 ID
                                    if tid in top_logprobs:
                                        ...
                                        if top_logprobs[tid].logprob > best_logprob:
                                            ...
                                            found = True
                                sample_probs[opt_char] = best_logprob if found else None    # 存入结果，如果 Top-20 里没找到，就存 None 或者 -999
                            batch_logprobs.append(sample_probs)
                        else:
                            batch_logprobs.append(None)
            else:
                outputs = []
                batch_logprobs = [] # 初始化一个空的 logprobs 列表，防止下面 crash
            ...
            #     if self.dataset_name in ["VideoMMMU", "MLVU", "LVBench"]:
            #         try:
