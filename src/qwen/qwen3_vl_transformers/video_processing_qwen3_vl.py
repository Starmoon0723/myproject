...
class Qwen3VLVideoProcessor(BaseVideoProcessor):
    ...
    size = {"shortest_edge": 128 * 32 * 32, "longest_edge": 32 * 32 * 768}  # 最小像素预算，最大像素预算
    ...
    # 两个参数
    # 已经做完 resize / (rescale+normalize) / patchify / merge 重排后，形成的“视频 patch token 序列”（实际上每个 token 是一个 patch 的像素向量，还没过视觉 encoder）。
    # 每个视频对应的 网格尺寸 [grid_t, grid_h, grid_w]（用来做位置编码/还原结构等）。
    ...
    def sample_frames(  # 把“抽帧策略”变成“要取哪些帧的索引”
        self,
